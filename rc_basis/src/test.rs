...
pub fn get_test_file_path(file_name: &str) -> String {
    let file_path = std::env::current_dir()
        ...
        .unwrap()
        .to_string();
    crate::files::ensure_parent_dir_exist(file_path.as_str());
    file_path.to_string()
}
...
pub fn save_and_open_file(file_name: &str, bytes: &[u8]) {
    ...
    crate::files::save_bytes_to_file(file_path.as_str(), bytes).unwrap();
    crate::files::open_file_with_sys(&file_path);
